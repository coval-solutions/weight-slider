...
class WeightSliderInternal extends StatelessWidget {
  ...
  int _indexToValue(int index) => (minValue + (index - 1)).round();

  @override
  build(BuildContext context) {
    int itemCount = ((maxValue - minValue) + 3).round();
  ...
  _animateTo(int valueToSelect, {int durationMillis = 200}) {
    double targetExtent = ((valueToSelect - minValue) * itemExtent).round();
  ...
  int _offsetToMiddleValue(double offset) {
    ...
    middleValue = math.max((minValue).round(), math.min((maxValue).round(), middleValue));
